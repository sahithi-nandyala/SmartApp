...
public class MetricCalculationImpl implements IMetricCalculation {
	...
	public Double calculateGraphDensity(Graph<MyNode, MyEdge> graph) {
		...
		@Override
		public Map<String, Double> clusteringCoefficient(Graph<MyNode, MyEdge> graph,LinkedList<MyNode> nodes) {
			
			Map<String, Double> coefficients = new HashMap<String, Double>();
			
			Map<MyNode, Double> results=Metrics.clusteringCoefficients(graph);
			
			for (int i = 0; i < nodes.size(); i++) {
				coefficients.put(nodes.get(i).getNodeId(),
						results.get(nodes.get(i)));
			}
